        ...
        <div className={styles.mainDivLogin}>
            ...
            <ThemeProvider theme={theme}>
                <Box component="form" sx={{ '& > :not(style)': { m: 1, width: '25ch' }, }} noValidate autoComplete="off" >
                    <TextField  label="Name" color="secondary" focused className={styles.textInputField} inputProps={{ style: { fontFamily: 'Arial', color: 'cyan'}}} onChange={(e) => setName(e.target.value)}  type="text" value={name}/>
                    <TextField label="Email" color="secondary" focused className={styles.textInputField}
                    inputProps={{ style: { fontFamily: 'Arial', color: 'cyan'}}} onChange={(e) => setEmail(e.target.value)}  type="email" value={email}/>
                    <TextField  label="Password" color="secondary" focused className={styles.textInputField} inputProps={{ style: { fontFamily: 'Arial', color: 'cyan'}}} onChange={(e) => setPassword(e.target.value)}  type="password" value={password}/>
                </Box>
                <div className={styles.btnLoginDiv}>
                    <button  onClick={handleSubmit}  className={styles.loginBtn}><span /><span /><span /><span /> Register</button>
                </div>
                
                </ThemeProvider>
                <Link to="/login" style={{color:"cyan",textDecoration:"none",marginLeft:"25px"}}>Already Registered With Us?</Link>
            </div>
        </div>
    )
}
